...
ファイル: `/app/data/routes-data.json`（Volume: `iot_fiware_gps_data`）
...
[
  {
    ...
    "authorName": "tanaka",
    "points": [
      { "lat": 43.0646, "lng": 141.3468 },
      { "lat": 43.0650, "lng": 141.3472 }
    ],
    "startTime": "2026-07-07T09:00:00.000Z",
    "endTime": "2026-07-07T09:30:00.000Z"
  }
]
```

ページ読み込み時に全デバイスのルートを取得し、地図上にポリライン＋スタート（🚩）/エンド（🏁）フラグマーカーで描画。
...
| GET | `/routes-list` | 全デバイスのルート一覧 `[{id, deviceId, authorName, points, startTime, endTime}]` |
| POST | `/route` | ルート保存 `{deviceId, authorName, points, startTime, endTime}` |
...
| ルート記録・共有 | ✅ 完了 | routes-data.json 永続化、全デバイス共有、スタート/エンド旗マーカー |
| 写真アップロード・地図表示 | ✅ 完了 | 位置情報付き、自分/他者色分け、表示切替 |
| 写真コメント（チャット形式） | ✅ 完了 | デバイス間共有、ポップアップ外ボタンで開閉、投稿者名表示 |
| 写真星評価 | ✅ 完了 | 1〜5 星、本人のみ変更可・他デバイスは参照のみ（DOM要素方式で初回クリックから確実に動作） |
